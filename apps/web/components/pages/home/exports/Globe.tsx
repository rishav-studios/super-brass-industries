...
// Degrees to shift the focused country upward on screen (avoids overlay)
const FOCUS_LATITUDE_OFFSET = THREE.MathUtils.degToRad(25);
...
export default function InteractiveGlobe({ highlightedCountries, className, selectedCountry }: Props) {
    ...
    const applyFocus = useCallback((name: string) => {
        ...
        // Tip the globe so the country appears above center (not hidden by overlay)
        const qOffset = new THREE.Quaternion().setFromAxisAngle(
            new THREE.Vector3(1, 0, 0),
            -FOCUS_LATITUDE_OFFSET
        );
        targetQuat.premultiply(qOffset);
    ...
    useEffect(() => {
        ...
        const onResize = () => {
            ...
            camera.setViewOffset(w, h * 2, 0, 0, w, h);
